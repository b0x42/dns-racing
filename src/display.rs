...
pub fn banner(cfg: &Args, servers: &[Server], csv_path: &str) {
    ...
    println!(
        "  {:<12} {} req/s per server  →  {} total",
        "Rate", cfg.rps, total_rps
    );
...
pub fn live_stats(
    ...
    lines.push(hr("└", "┴", "┘"));
    lines.push(format!(
        "  {YELLOW}{BOLD}Stop the race with ESC or Ctrl+C{RESET}"
    ));
...
pub fn domain_breakdown(store: &Store, servers: &[Server]) {
    ...
    let hr = |l: &str, m: &str, r: &str| -> String {
        ...
        for w in &col_w {
            s += &format!("{m}{}", "─".repeat(w + 2));
        }
        s += &format!("{m}{}{r}", "─".repeat(diff_col + 2));
        s
    };

    let title: Vec<String> = servers
        .iter()
        .map(|s| format!("{}{}{RESET}", s.color, s.label))
        .collect();
    println!(
...
pub fn verdict(store: &Store, servers: &[Server]) {
    ...
    for (pos, &(idx, avg, p95, min)) in ranked.iter().enumerate() {
        ...
        let diff = if pos == 0 {
            ...
            format!("+{}", fmt_ms(avg - fastest))
        };
        let rank = ranks
            .get(pos)
            .map(|r| r.to_string())
            .unwrap_or_else(|| format!("{}th", pos + 1));
        println!(
            "│{}│ {}{BOLD}{:<w$}{RESET} │{}│{}│{}│{}│",
            cell(&rank, cols[0]),
            s.color,
